...
MessageListWidget::MessageListWidget(QWidget* parent) : CustomScrollArea(parent) {
    ...
    connect(&handler, &MessageHandler::unreadMessageReceived, this, &MessageListWidget::onUnreadMessageReceived);
    connect(&handler, &MessageHandler::updateLastMessageTime, this, &MessageListWidget::onLastMessageTimeUpdated);

    // 连接好友请求接受信号
    connect(&NetworkManager::instance(), &NetworkManager::friendRequestAccepted, this, &MessageListWidget::updateMessages);
}
...
void MessageListWidget::updateMessages() {
    ...
    for (const auto& group : groups) {
        ...
        connect(item, &MessageListItem::itemClicked, this, &MessageListWidget::onItemClicked);
        m_items.append(item);
    }
...
void MessageListWidget::onUnreadMessageReceived(const QString& conversationId, const QString& content, int /* unreadCount */) {
    // 查找对应的消息列表项
...
void MessageListWidget::onLastMessageTimeUpdated(const QString& chatId, const QDateTime& timestamp) {
