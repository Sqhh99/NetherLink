...
void ChatItemDelegate::paint(QPainter* painter, const QStyleOptionViewItem& option, const QModelIndex& index) const {
    ...
    if (timeHeader) {
        // 绘制时间标识
        QRect timeHeaderRect = QRect(0, // x
...
bool ChatItemDelegate::editorEvent(QEvent* event, QAbstractItemModel* model, const QStyleOptionViewItem& option, const QModelIndex& index) {
...
void ChatItemDelegate::drawBubble(QPainter* painter, const QRect& rect, bool isFromMe, const ChatMessage* message, bool isSelected) const {
    // 设置气泡颜色
    QColor bubbleColor;
...
void ChatItemDelegate::drawAvatar(QPainter* painter, const QRect& rect, const QString& userID) const {
    QPixmap avatar = AvatarLoader::instance().getAvatar(userID);

    painter->drawPixmap(rect, avatar);
}

void ChatItemDelegate::drawTextMessage(QPainter* painter, const QRect& rect, const QString& text, bool isFromMe, bool isSelected) const {
...
void ChatItemDelegate::drawImageMessage(QPainter* painter, const QRect& rect, const QPixmap& image, bool /* isFromMe */) const {
    if (image.isNull())
        return;
    QRect imageRect = rect.adjusted(BUBBLE_PADDING, BUBBLE_PADDING, -BUBBLE_PADDING, -BUBBLE_PADDING);

    QSize scaledSize = image.size().scaled(imageRect.size(), Qt::KeepAspectRatio);
    QRect targetRect(imageRect.x() + (imageRect.width() - scaledSize.width()) / 2, imageRect.y() + (imageRect.height() - scaledSize.height()) / 2, scaledSize.width(), scaledSize.height());

    painter->drawPixmap(targetRect, image);
}

void ChatItemDelegate::drawGroupInfo(QPainter* painter, const QRect& rect, const ChatMessage* message) const {
    painter->save();
...
void ChatItemDelegate::drawGroupInfoForMe(QPainter* painter, const QRect& rect, const ChatMessage* message) const {
    painter->save();
    ...
    if (role != GroupRole::Member) {
        ...
        QRect roleRect(startX, rect.top() + (rect.height() - ROLE_HEIGHT) / 2, roleWidth, ROLE_HEIGHT);
...
QSize ChatItemDelegate::sizeHint(const QStyleOptionViewItem& option, const QModelIndex& index) const {
    ...
    if (timeHeader) {
        ...
        if (message->getType() == MessageType::Text) {
            ...
            if (!image.isNull()) {
                QSize scaledSize = image.size().scaled(maxBubbleWidth - 2 * BUBBLE_PADDING, 200, Qt::KeepAspectRatio);
...
QRect ChatItemDelegate::calculateBubbleRect(const QRect& contentRect, const ChatMessage* message, int maxWidth, bool isFromMe) const {
    // 使用更大的字体
    QFont messageFont = QApplication::font();
    ...
    if (message->getType() == MessageType::Text) {
        ...
        if (!image.isNull()) {
            QSize scaledSize = image.size().scaled(maxWidth - 2 * BUBBLE_PADDING, 200, Qt::KeepAspectRatio);
    ...
    int x = isFromMe ? contentRect.right() - bubbleWidth - AVATAR_SIZE - BUBBLE_MARGIN : AVATAR_SIZE + BUBBLE_MARGIN + LEFT_MARGIN;

    return (QRect(x, contentRect.y() + BUBBLE_MARGIN, bubbleWidth, bubbleHeight));
}

QRect ChatItemDelegate::calculateAvatarRect(const QRect& contentRect, bool isFromMe) const {
...
void ChatItemDelegate::showContextMenu(const QPoint& pos, const QModelIndex& index, const ChatMessage* message) const {
...
void ChatItemDelegate::drawTimeHeader(QPainter* painter, const QRect& rect, const QString& text) const {
    // 设置字体
    ...
    QRect timeRect((rect.width() - totalWidth) / 2, // 水平居中
                   rect.y(), // 使用传入的y坐标
                   totalWidth, TIME_HEADER_HEIGHT);
...
QRect ChatItemDelegate::calculateTimeHeaderRect(const QRect& contentRect, const QString& text) const {
    QFont font = QApplication::font();
    ...
    // 计算文本宽度，确保最小宽度
    int textWidth = qMax(fm.horizontalAdvance(text) + 2 * TIME_HEADER_PADDING, TIME_HEADER_MIN_WIDTH);
...
void ChatItemDelegate::onAvatarLoaded(const QString& /* userId */, const QPixmap& /* pixmap */) {
    // 通知视图更新
