...
ChatListView::ChatListView(QWidget*parent) : QListView(parent), m_smoothScrollValue(0) {
    ...
    // 连接信号槽
    connect(customScrollBar, &SmoothScrollBar::valueChanged, this, &ChatListView::onCustomScrollValueChanged);
...
void ChatListView::setModel(QAbstractItemModel*model) {
    if (this->model()) {
        ...
        disconnect(this->model(), &QAbstractItemModel::modelReset, this, &ChatListView::onModelRowsChanged);
    }
    QListView::setModel(model);

    if (model) {
        connect(model, &QAbstractItemModel::rowsInserted, this, &ChatListView::onModelRowsChanged);
        connect(model, &QAbstractItemModel::rowsRemoved, this, &ChatListView::onModelRowsChanged);
        connect(model, &QAbstractItemModel::modelReset, this, &ChatListView::onModelRowsChanged);
