...
void NotificationListItem::setupUI() {
    ...
    actionButton->setStyleSheet("QPushButton {""   background-color: #ffffff;""   border: 1px solid #cccccc;""   border-radius: 4px;""}""QPushButton:hover {""   background-color: #f0f0f0;""}""QPushButton:pressed {""   background-color: #e0e0e0;""}");
}
...
void NotificationListItem::resizeEvent(QResizeEvent* event) {
    ...
    // 布局操作按钮
    actionButton->move(width() - padding - actionButton->width(), (height() - actionButton->height()) / 2);
}
