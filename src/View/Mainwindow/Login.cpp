        ...
        : FramelessWindow(parent)
{
    ...
    this->userHead->setPixmap(icon.scaled(
...
bool Login::eventFilter(QObject* target, QEvent* event)
{
    if (target == this->closeButton) {
        if (event->type() == QEvent::MouseButtonPress) {
            QTimer::singleShot(150, this, &Login::close);
            return true;
        }
    }
    if (target == this->registerButton) {
        if (event->type() == QEvent::MouseButtonPress) {
            ...
            // 连接注册成功信号
            connect(registerWindow, &Register::registerSuccess,
                    this, &Login::setLoginInfo);
...
void Login::doLogin(QString account, QString password)
{
    ...
    connect(reply, &QNetworkReply::finished, this, [this, reply, manager, timeoutTimer]() {
        ...
        // 5) 处理WebSocket连接成功
        connect(&NetworkManager::instance(), &NetworkManager::wssConnected,
                this, [this, uid, token]() {
                    onLoginSuccess(uid, token);
                }, Qt::SingleShotConnection);

        // 6) 处理WebSocket错误
        connect(&NetworkManager::instance(), &NetworkManager::wssError,
                this, [this, reply, manager, timeoutTimer](const QString& err) {
                    NotificationManager::instance().showMessage(
                            QString("WebSocket连接失败：%1").arg(err),
                            NotificationManager::Error,
                            this
                    );
                    if (reply) {
...
void Login::onLoginSuccess(const QString& uid, const QString& token)
{
    ...
    if (!contactsWatcher) {
        ...
        connect(contactsWatcher, &QFutureWatcher<void>::finished,
                this, &Login::onContactsLoaded);
    }
