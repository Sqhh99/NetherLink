...
#include "View/Mainwindow/ApplicationBarItem.h"
...
ApplicationBarItem::ApplicationBarItem(QPixmap normal, QWidget* parent) : ApplicationBarItem(normal, normal, parent) {}
...
void ApplicationBarItem::paintEvent(QPaintEvent*) {
    ...
    QPixmap normalScaled = base.scaled(tgtF.toSize(), Qt::KeepAspectRatio, Qt::SmoothTransformation);
    ...
    if (selected && (rippleRadius > 0.0)) {
        ...
        clipPath.addEllipse(QRectF(-rippleRadius, height() - rippleRadius, rippleRadius * 2, rippleRadius * 2));
        painter.setClipPath(clipPath);

        // 在剪裁区域内绘制被选中状态的图标
        QPixmap selScaled = selectedPixmap.scaled(tgtF.toSize(), Qt::KeepAspectRatio, Qt::SmoothTransformation);
