    ...
    : FramelessWindow(parent)
{
    ...
    defaultAvatar = defaultAvatar.scaled(
            QSize(this->userHead->size()) * 1.5,
    ...
    connect(this->registerButton, &CustomPushButton::clicked, this, [this]() {
        ...
        QString password = this->passwordEdit->currentText();
        ...
        // 验证通过，发送注册请求
        doRegister(email, code, password, nickname);
    });
...
void Register::requestVerificationCode(const QString& email)
{
    ...
    connect(reply, &QNetworkReply::finished, this, [this, reply, manager]() {
        ...
        if (reply->error() != QNetworkReply::NoError && statusCode != 400) {
            NotificationManager::instance().showMessage(
                    QString("获取验证码失败：网络错误 - %1").arg(reply->errorString()),
                    NotificationManager::Error,
                    this
            );
        ...
        if (statusCode == 400 || obj.contains("error")) {
            ...
            NotificationManager::instance().showMessage(
                    QString("获取验证码失败：%1").arg(errorMsg),
                    NotificationManager::Error,
                    this
            );
            getCodeButton->setEnabled(true);
...
void Register::doRegister(const QString& email, const QString& code, const QString& password, const QString& nickname)
{
    if (isRegistering) return;
    isRegistering = true;
    ...
    connect(reply, &QNetworkReply::finished, this, [this, reply, manager]() {
        ...
        if (reply->error() != QNetworkReply::NoError && statusCode != 400) {
            NotificationManager::instance().showMessage(
                    QString("注册失败：网络错误 - %1").arg(reply->errorString()),
                    NotificationManager::Error,
                    this
            );
            cleanup();
...
void Register::uploadAvatar(const QString& filePath)
{
    ...
    // 将正方形图片保存到临时文件
    QString tempPath = QDir::tempPath() + "/temp_avatar_" +
    ...
    connect(reply, &QNetworkReply::finished, this, [this, reply, tempPath, manager]() {
        ...
        if (obj.contains("error")) {
            NotificationManager::instance().showMessage(
                    QString("上传失败：%1").arg(obj["error"].toString()),
                    NotificationManager::Error, this
            );
            return;
        }
        ...
        if (obj.contains("url")) {
            ...
            NotificationManager::instance().showMessage(
                    "头像上传成功",
                    NotificationManager::Success, this
            );
        }
    });
}
