...
PostApplicationBar::PostApplicationBar(QWidget* parent) : QWidget(parent), m_parent(parent) {
    ...
    connect(highlightAnim, &QVariantAnimation::valueChanged, this, &PostApplicationBar::onHighlightValueChanged);
    initItems();
...
void PostApplicationBar::initItems() {
    QStringList labels = {"首页", "朋友圈", "发表", "消息", "我"};
...
void PostApplicationBar::layoutItems() {
    ...
    for (int i = 0; i < items.size(); ++i) {
        ...
        items[i]->setGeometry(startX, y, w, itemHeight);
        startX += w + spacing;
    }
}
