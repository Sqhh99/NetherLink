...
void PostPreviewItem::downloadAndSetupImages() {
    ...
    connect(reply, &QNetworkReply::finished, this, [this, reply] () {
        if (reply->error() == QNetworkReply::NoError) {
            ...
            int h0 = int(w0 / ratio);
            QPixmap tmp = origPostImage.scaled(w0, h0, Qt::IgnoreAspectRatio, Qt::SmoothTransformation);
...
void PostPreviewItem::setupUI() {
    ...
    if (!QPixmapCache::find("full_heart", &fullHeart)) {
        fullHeart = QPixmap(":/icon/full_heart.png").scaled(20, 20, Qt::KeepAspectRatio, Qt::SmoothTransformation);
        QPixmapCache::insert("full_heart", fullHeart);
    }

    if (!QPixmapCache::find("empty_heart", &emptyHeart)) {
        emptyHeart = QPixmap(":/icon/heart.png").scaled(20, 20, Qt::KeepAspectRatio, Qt::SmoothTransformation);
...
void PostPreviewItem::resizeEvent(QResizeEvent* ev) {
    ...
    if (!m_croppedPostImage.isNull()) {
        m_imageLabel->setRoundedPixmap(m_croppedPostImage.scaled(W, imgH, Qt::KeepAspectRatio, Qt::SmoothTransformation), 12);
    }
    currentY += imgH;
    ...
    QRect rText = fm.boundingRect(0, 0, W - 2 * Margin, maxTitleH, Qt::TextWordWrap, m_titleText);
    ...
    m_likeIconLabel->setGeometry(likeIconX, currentY + (AvatarR - iconW) / 2, iconW, iconW);
    m_likeCountLabel->setGeometry(W - Margin - countW, currentY, countW, AvatarR);

    int authorX = AvatarR + Margin + Margin;

    m_authorLabel->setGeometry(authorX, currentY, likeIconX - authorX - Margin, AvatarR);
