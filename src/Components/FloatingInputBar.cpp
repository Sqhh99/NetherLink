...
void FloatingInputBar::initUI() {
    ...
    m_inputEdit->setStyleSheet("QTextEdit {""   background-color: transparent;""   border: none;""   color: #333333;""   font-size: 15px;""   font-family: 'Microsoft YaHei', '微软雅黑';""   padding: 5px;""   selection-background-color: #3064CE;""   selection-color: white;""}""QTextEdit::cursor {""   border-left: 2px solid #3064CE;""}""QScrollBar:vertical {""   border: none;""   background: transparent;""   width: 8px;""   margin: 0px;""}""QScrollBar::handle:vertical {""   background: rgba(0, 0, 0, 0.2);""   border-radius: 4px;""   min-height: 20px;""}""QScrollBar::add-line:vertical, QScrollBar::sub-line:vertical {""   height: 0px;""}""QScrollBar::add-page:vertical, QScrollBar::sub-page:vertical {""   background: none;""}");
    ...
    updateLabelIcon(m_sendLabel, "book_and_pen.png", "hovered_book_and_pen.png", QSize(SEND_BUTTON_SIZE, SEND_BUTTON_SIZE));
...
void FloatingInputBar::updateLabelIcon(QLabel*label, const QString &normalIcon, const QString &hoveredIcon, const QSize &size) {
    ...
    painter.drawPixmap((size.width() - targetSize.width()) / 2, (size.height() - targetSize.height()) / 2, scaledNormal);
...
void FloatingInputBar::handleLabelHover(QLabel*label, const QString & /*normalIcon*/, const QString &hoveredIcon, const QSize &size) {
    ...
    painter.setRenderHint(QPainter::SmoothPixmapTransform);
    painter.drawPixmap((size.width() - targetSize.width()) / 2, (size.height() - targetSize.height()) / 2, pixmap.scaled(targetSize, Qt::KeepAspectRatio, Qt::SmoothTransformation));
    label->setPixmap(finalIcon);
}
...
void FloatingInputBar::resizeEvent(QResizeEvent*event) {
    ...
    // 更新发送按钮位置
    if (m_sendLabel) {
        m_sendLabel->move(width() - SEND_BUTTON_SIZE - RIGHT_MARGIN, height() - SEND_BUTTON_SIZE - VERTICAL_MARGIN);
...
void FloatingInputBar::mousePressEvent(QMouseEvent*event) {
    ...
    if (m_sendLabel && m_sendLabel->geometry().contains(pos)) {
        ...
        QString filePath = QFileDialog::getOpenFileName(this, tr("选择图片"), "", tr("图片文件 (*.png *.jpg *.jpeg *.bmp)"));
...
void FloatingInputBar::showTooltip(QLabel*label, const QString &text) {
    ...
    QPoint tooltipPos(labelPos.x() + label->width() / 2 - m_tooltip->width() / 2, labelPos.y());
