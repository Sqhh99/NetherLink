...
QSharedPointer <ChatMessage> MessageRepository::createMessageFromQuery(QSqlQuery& query, bool /*  isGroupChat */) {
    ...
    if (type == "text") {
        message = QSharedPointer <ChatMessage>(new TextMessage(content, isFromMe, senderId, msgIsGroupChat, senderName, role));
    } else if (type == "image") {
        QPixmap pixmap;
        QString localPath;
        ...
        auto imageMsg = QSharedPointer <ImageMessage>(new ImageMessage(pixmap, isFromMe, senderId, msgIsGroupChat, senderName, role));
...
QVector <QSharedPointer <ChatMessage> > MessageRepository::getMessages(const QString& conversationId, bool isGroupChat) {
    ...
    if (query.exec()) {
        ...
        qWarning() << "Failed to get messages for conversation:" << conversationId << "Error:" << query.lastError().text();
    }
...
void MessageRepository::addMessage(const QString& conversationId, QSharedPointer <ChatMessage> message) {
    if (!message) {
        return;
    }
    ...
    query.prepare("INSERT INTO message (id, conversation, sender_id, timestamp, type, content, extra, isGroupChat) ""VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
